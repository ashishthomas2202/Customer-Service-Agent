...
const getProduct = tool({
  ...
  parameters: z.object({ sku: z.string(), name_query: z.string() }),
  async execute({ sku, name_query }) {
    try {
      ...
      if (s) {
        rows = await q(
          `SELECT SKU, NAME, CATEGORY, PRICE, SHORT_DESC FROM ${T(
            "PRODUCTS"
      ...
      const p = rows[0];
      const specs = await q(
        `SELECT SPEC_KEY, SPEC_VAL FROM ${T(
          "PRODUCT_SPECS"
        )} WHERE SKU = ? ORDER BY SPEC_KEY`,
        [p.SKU]
      );
      ...
      if (String(e?.message) === "db_unavailable")
...
const getCustomer = tool({
  ...
  parameters: z.object({ customer_id: z.number(), email: z.string() }),
  async execute({ customer_id, email }) {
    try {
      ...
      const rows =
        id > 0
          ? await q(
              `SELECT CUSTOMER_ID, NAME, EMAIL, PHONE, TIER, LIFETIME_VALUE FROM ${T(
                "CUSTOMERS"
              )} WHERE CUSTOMER_ID = ?`,
              [id]
            )
...
const getOrder = tool({
  ...
  async execute({ order_no }) {
    try {
      ...
      const cust = await q(
        `SELECT CUSTOMER_ID, NAME, EMAIL, PHONE, TIER FROM ${T(
          "CUSTOMERS"
        )} WHERE CUSTOMER_ID = ?`,
        [o.CUSTOMER_ID]
      );
  ...
  async execute({ order_no }) {
    try {
      ...
      const events = await q(
        `SELECT TS, CODE, LOCATION, NOTE FROM ${T(
          "SHIP_EVENTS"
        )} WHERE TRACKING_NO = ? ORDER BY TS`,
        [o.TRACKING_NO]
      );
...
const addOrderNote = tool({
  ...
  needsApproval: true,
  parameters: z.object({ order_no: z.number(), note: z.string().min(3) }),
