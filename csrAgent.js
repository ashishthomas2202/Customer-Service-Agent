...
import { z } from "zod";
import { getDb, invalidateDb } from "./utils/mapepire.js";
...
async function ensureSchema() {
  ...
  if (!db) return false; // let caller decide
  try {
...
// Query helper with self-heal on transport/pool errors
async function q(sql, params = []) {
  ...
  try {
    ...
    // Pool or transport blew up → drop cached pool and retry once
    if (
      msg.includes("getRunningCount") || // half-initialized pool
      msg.includes("Connection failed with code 1006") ||
      msg.includes("ECONNRESET") ||
      msg.includes("ETIMEDOUT")
    ) {
      console.warn("DB looked unhealthy, invalidating and retrying once…", msg);
      invalidateDb();
      db = await getDb();
      if (!db) throw new Error("db_unavailable");
      const res = await db.query(sql, params);
